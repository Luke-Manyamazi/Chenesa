...
const deleteCards = [
  ...
  { icon: Megaphone, label: 'Marketing & Newsletters',  desc: 'Promotions, sales, "we miss you" emails — permanently deleted.',   color: 'text-orange-400', bg: 'bg-orange-500/10 border-orange-500/20' },
  ...
  { icon: Clock,     label: 'Old Read Emails',          desc: 'Emails you\'ve already read and no longer need — removed.',        color: 'text-slate-400',  bg: 'bg-slate-500/10  border-slate-500/20'  },
]
...
const steps = [
  { n: '01', icon: Mail,   title: 'Connect your email',         desc: 'Sign in with Gmail or enter an App Password for any other provider. Takes 2 minutes.',                                         color: 'from-blue-950/60    border-blue-500/20    text-blue-400'    },
  { n: '02', icon: Zap,    title: 'AI reads & classifies',      desc: 'Chenesa reads subject lines and senders — never the full email body. Claude AI classifies each email instantly.',               color: 'from-primary-950/60 border-primary-500/20 text-primary-400' },
  { n: '03', icon: Shield, title: 'Junk deleted, important kept',desc: 'Spam, marketing, and clutter are permanently deleted. Work, financial, and personal emails are always protected.',             color: 'from-green-950/60   border-green-500/20   text-green-400'   },
]

const providers = ['Gmail', 'Outlook', 'Yahoo', 'iCloud', 'AOL', 'Zoho', 'Any IMAP']
...
const plans = [
  {
    ...
    features: ['3 free cleaning runs', '100 emails per run', '1 email account', 'AI classification', 'Cleaning history'],
    cta: 'Start free', href: '/signup', ctaStyle: 'border border-border hover:border-primary-500/60 text-slate-300 hover:text-white',
  },
  {
    name: 'Basic', price: '$4', period: '/mo', highlight: false,
    badge: 'bg-blue-500/10 text-blue-400 border-blue-500/20',
    accent: 'text-blue-400',
    features: ['Unlimited cleaning runs', '500 emails per run', '2 email accounts', 'Auto-scheduler (6h)', 'Full history & AI reasoning', 'Keep rules'],
    cta: 'Coming soon', href: '/upgrade', ctaStyle: 'border border-border text-slate-500 cursor-not-allowed',
  },
  {
    name: 'Pro', price: '$9', period: '/mo', highlight: true,
    badge: 'bg-primary-500/10 text-primary-400 border-primary-500/20',
    accent: 'text-primary-400',
    features: ['Unlimited cleaning runs', '2,000 emails per run', '5 email accounts', 'Auto-scheduler (6h)', 'Full history & AI reasoning', 'Keep rules', 'Priority support'],
    cta: 'Coming soon', href: '/upgrade', ctaStyle: 'border border-border text-slate-500 cursor-not-allowed',
  },
]
    ...
    <div className="bg-background text-slate-100 overflow-x-hidden">
      ...
      <section className="relative mx-auto max-w-6xl px-4 pt-20 pb-24 sm:pt-28 sm:pb-32 text-center">
        ...
        <div className="inline-flex items-center gap-2 rounded-full border border-primary-700/50
          ...
          Works with Gmail, Outlook, Yahoo &amp; more
        </div>
        ...
        <p className="mx-auto max-w-2xl text-lg sm:text-xl text-slate-400 leading-relaxed mb-10">
          Chenesa connects to any email account and uses Claude AI to delete spam,
          marketing emails, and old clutter — automatically, every day.
          You keep the important stuff. We handle the rest.
        </p>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-16">
          <Link href="/signup"
            ...
            <Zap size={16} className="fill-white" /> Start free — no card needed
          </Link>
          <a href="#how"
            className="w-full sm:w-auto inline-flex items-center justify-center gap-2
        ...
        <div className="mx-auto max-w-sm">
          <div className="rounded-2xl border border-green-500/30 bg-gradient-to-br from-green-950/40 to-slate-900 p-5 text-left shadow-2xl shadow-green-900/20">
            <div className="flex items-center gap-3 mb-5">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-green-400 font-medium flex items-center gap-1">
                  <CheckCircle size={10} /> Clean complete
                </p>
              </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="rounded-xl bg-slate-800/60 p-3 text-center">
                <p className="text-3xl font-bold text-slate-300">234</p>
                <p className="text-xs text-slate-400 mt-0.5">✓ kept</p>
              </div>
            </div>
          </div>
          <p className="text-xs text-slate-600 mt-3">Real result — 2,081 emails processed in under 2 minutes</p>
        </div>
      </section>
      ...
      <section className="border-y border-border bg-surface/40 py-5">
        <div className="mx-auto max-w-6xl px-4">
          ...
          <div className="flex flex-wrap items-center justify-center gap-3">
            {providers.map(p => (
              <span key={p}
                className="rounded-full border border-border bg-surface px-4 py-1.5 text-sm text-slate-400
                  hover:border-primary-500/40 hover:text-slate-200 transition-colors">
                {p}
              </span>
            ))}
          </div>
      ...
      <section className="mx-auto max-w-6xl px-4 py-16">
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[
            { value: '10M+',   label: 'Emails cleaned',      color: 'text-red-400',     border: 'border-red-500/20    from-red-950/30'    },
            { value: '< 2min', label: 'Average run time',    color: 'text-primary-400', border: 'border-primary-500/20 from-primary-950/30' },
            { value: '99%',    label: 'Accuracy rate',       color: 'text-green-400',   border: 'border-green-500/20  from-green-950/30'  },
            { value: 'Any',    label: 'Email provider',      color: 'text-blue-400',    border: 'border-blue-500/20   from-blue-950/30'   },
          ].map(s => (
            <div key={s.label}
      ...
      <section className="border-t border-border bg-surface/20 py-20">
        <div className="mx-auto max-w-6xl px-4">
          <div className="text-center mb-12">
            ...
            <h2 className="text-3xl sm:text-4xl font-bold text-white">AI that knows the difference</h2>
          </div>
          <div className="grid gap-8 lg:grid-cols-2">
            ...
            <div>
              <div className="flex items-center gap-2 mb-5">
                ...
                <h3 className="font-bold text-white text-lg">What gets deleted</h3>
              </div>
              ...
              <div className="rounded-2xl border border-green-800/30 bg-gradient-to-br from-green-950/20 to-slate-900/50 p-6 h-full">
                ...
                <div className="mt-5 pt-4 border-t border-green-900/40">
                  <p className="text-xs text-slate-500 leading-relaxed">
                    ...
                    keywords that always protect matching emails regardless of AI classification.
                  </p>
                </div>
              </div>
            </div>
      ...
      <section className="border-t border-border bg-surface/20 py-20">
        <div className="mx-auto max-w-3xl px-4 text-center">
          <div className="inline-flex items-center gap-2 mb-6 px-4 py-2 rounded-full
            ...
            <BarChart3 size={14} /> Join users already cleaning their inbox
          </div>
          <h2 className="text-3xl sm:text-4xl font-bold text-white mb-4">
            Ready to reclaim your inbox?
          </h2>
          <p className="text-slate-400 mb-10 text-lg">
            3 free cleans. No credit card. Takes 2 minutes to set up.
          </p>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <Link href="/signup"
              className="w-full sm:w-auto inline-flex items-center justify-center gap-2
                ...
                px-8 py-4 text-base font-semibold text-white
                hover:opacity-90 transition-opacity shadow-xl shadow-primary-900/50">
              <Zap size={16} className="fill-white" /> Get started free
            </Link>
            <Link href="/login"
              className="w-full sm:w-auto inline-flex items-center justify-center gap-2
                rounded-xl border border-border px-8 py-4 text-base font-medium text-slate-300
